...
1. [Установите Hugo](https://gohugo.io/installation/) на своём компьютере. Выполните команду
```bash
hugo server
```
**Либо** в без установки, докере:
```bash
docker run --rm -v ${PWD}:/project -p 1313:1313 ghcr.io/gohugoio/hugo:latest server --bind 0.0.0.0
```
2. Hugo запустит локальный веб-сервер. Найдите в выводе команды строку `Web Server is available at http://localhost:1313/ (bind address 127.0.0.1)`. Перейдите по указанной ссылке.
3. Можете менять файлы сайта и смотреть, как это влияет на результат. Hugo автоматически отслеживает изменения, перезапускать сервер не нужно.
